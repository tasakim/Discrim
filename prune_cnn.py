...
if __name__ == '__main__':
    ...
    for (name, module) in pruned_model.named_modules():
        if isinstance(module, nn.Conv2d):
            if conv_count in l1:
                ...
                unmask, mask, score = manner_list[args.manner](module.weight.data, discrim, criterion_rec, max_dim, ratio,
                                                        args)
                score_ += score.tolist()
                layer_score_id += 1
                conv_count += 1
                ...
                conv_count += 1
    length = len(score_)
    threshold = np.sort(score_)[int(length*0.65)]
    print(threshold)
    layer_score_id = 0
    conv_count = 1
    mask_index = []
    for (name, module) in pruned_model.named_modules():
        if isinstance(module, nn.Conv2d):
            if conv_count in l1:
                ...
                mask_index.append(mask)
                # mask_index.append(mask.cpu())
                # print(mask_index[-1].device, module.weight.data.device)
                device = module.weight.device
                weight = torch.index_select(module.weight.data, dim=0, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
                bias = torch.index_select(module.bias.data, dim=0,
                                          index=torch.LongTensor(mask_index[-1]).to(module.weight.device)) if module.bias is not None else None
                ...
                unmask, mask = manner_list[args.manner](module.weight.data, discrim, criterion_rec, max_dim, ratio,
                                                        args, threshold=threshold)
                mask_index.append(mask.long().to(module.weight.device))
                ...
                weight = torch.index_select(module.weight.data, dim=0, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
                weight = torch.index_select(weight, dim=1, index=torch.LongTensor(mask_index[-1 - 1]).to(module.weight.device))
                bias = torch.index_select(module.bias.data, dim=0, index=torch.LongTensor(
                    mask_index[-1]).to(module.weight.device)) if module.bias is not None else None
                new_layer = nn.Conv2d(in_channels=weight.shape[1], out_channels=weight.shape[0],
                                      kernel_size=weight.shape[2],
                                      stride=module.stride, padding=module.padding, groups=module.groups,
                                      dilation=module.dilation, bias=module.bias)
                ...
                weight = torch.index_select(module.weight.data, dim=1, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
            ...
            if conv_count - 1 in l1 + l2:
                weight = torch.index_select(module.weight.data, dim=0, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
                bias = torch.index_select(module.bias.data, dim=0, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
                running_mean = torch.index_select(module.running_mean.data, dim=0,
                                                  index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
                running_var = torch.index_select(module.running_var.data, dim=0, index=torch.LongTensor(mask_index[-1]).to(module.weight.device))
    ...
    if dist.get_rank() == 0:
        ...
        ori_flops, ori_params = clever_format(profile(target_model.cpu(), inputs=(torch.randn(1, 3, size, size),)), '%.2f')
        pruned_flops, pruned_params = clever_format(profile(pruned_model.cpu(), inputs=(torch.randn(1, 3, size, size),)),
                                                    '%.2f')
