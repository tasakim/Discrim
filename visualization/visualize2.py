...
def hook_fn(module, input, output):
    ...
    if layer_index in layer_list:
        ...
        for index in range(old_weight.shape[0]):
            ...
            reconstructed_weight[index, ...] = new_weight[:, :, :length].reshape([1, shape[1], shape[2], shape[3]])
        ...
        for i in range(num_plot):
            ...
            plt.imshow(old_output.squeeze(0)[plot_index[i]].detach().cpu())
            plt.axis('off')
        # plt.savefig(r'./fig/{}_old.png'.format(layer_index))
        # plt.clf()
        for i in range(num_plot):
            plt.subplot(2, num_plot, i + num_plot + 1)
            plt.imshow(new_output.squeeze(0)[plot_index[i]].detach().cpu())
            plt.axis('off')
...
target_model = torchvision.models.resnet18()
target_model.load_state_dict(torch.load('../pretrained/r18_imagenet.pth'))
p_ratio = 0.5
layer_index = 1
fig_index = 0
